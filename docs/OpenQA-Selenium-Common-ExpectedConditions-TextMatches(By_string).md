...
## ExpectedConditions.TextMatches(By, string) Method
An expectation for the given element's text to match a pattern.  
```csharp
public static System.Func<IWebDriver,IWebElement> TextMatches(By by, string regex);
```
#### Parameters
<a name='OpenQA-Selenium-Common-ExpectedConditions-TextMatches(By_string)-by'></a>
...
<a name='OpenQA-Selenium-Common-ExpectedConditions-TextMatches(By_string)-regex'></a>
